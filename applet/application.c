...
static void application_append_node_r (GMenuTreeDirectory* node);
...
static char* application_generate_key (GMenuTreeEntry* entry);
...
void application_init (void) {
	...
	node = NULL;
	node = (_tmp0_ = _gmenu_tree_item_ref0 (gmenu_tree_get_root_directory (gmenu_tree_lookup ("applications.menu", GMENU_TREE_FLAGS_INCLUDE_EXCLUDED))), _gmenu_tree_item_unref0 (node), _tmp0_);
	application_append_node_r (node);
	node = (_tmp1_ = _gmenu_tree_item_ref0 (gmenu_tree_get_root_directory (gmenu_tree_lookup ("settings.menu", GMENU_TREE_FLAGS_INCLUDE_EXCLUDED))), _gmenu_tree_item_unref0 (node), _tmp1_);
	application_append_node_r (node);
...
static void application_append_node_r (GMenuTreeDirectory* node) {
	g_return_if_fail (node != NULL);
	{
		...
		GSList* item_it;
		item_collection = gmenu_tree_directory_get_contents (node);
		for (item_it = item_collection; item_it != NULL; item_it = item_it->next) {
			GMenuTreeItem* item;
			item = _gmenu_tree_item_ref0 ((GMenuTreeItem*) item_it->data);
			{
				switch (gmenu_tree_item_get_type (item)) {
					case GMENU_TREE_ITEM_ENTRY:
					{
						...
						char* _tmp0_;
						Application* _tmp1_;
						entry = _gmenu_tree_item_ref0 ((GMenuTreeEntry*) item);
						key = application_generate_key (entry);
						g_debug ("application.vala:200: gmenu key = %s", key);
						app = application_new ();
						app->priv->key = (_tmp0_ = g_strdup (key), _g_free0 (app->priv->key), _tmp0_);
						application_set_not_in_menu (app, FALSE);
						application_set_readable_name (app, gmenu_tree_entry_get_name (entry));
						application_set_exec_path (app, gmenu_tree_entry_get_exec (entry));
						application_set_icon_name (app, gmenu_tree_entry_get_icon (entry));
						g_hash_table_insert (application_dict, g_strdup (key), app);
						application_applications = g_list_prepend (application_applications, (_tmp1_ = app, app = NULL, _tmp1_));
						_application_unref0 (app);
						_g_free0 (key);
						_gmenu_tree_item_unref0 (entry);
						break;
					}
					case GMENU_TREE_ITEM_DIRECTORY:
					{
						application_append_node_r ((GMenuTreeDirectory*) item);
						break;
					}
				}
...
static void application_window_opened (WnckScreen* s, WnckWindow* wwin) {
	...
	g_object_set_data_full ((GObject*) wwin, "gnomenu-app", _application_ref0 (app), application_unref);
...
static void application_window_closed (WnckScreen* s, WnckWindow* wwin) {
	...
	g_return_if_fail (s != NULL);
	g_return_if_fail (wwin != NULL);
	app = _application_ref0 ((Application*) g_object_get_data ((GObject*) wwin, "gnomenu-app"));
	if (app == NULL) {
		_application_unref0 (app);
		return;
	}
...
static char* application_generate_key (GMenuTreeEntry* entry) {
	...
	exec = TRUE;
	{
		g_shell_parse_argv (gmenu_tree_entry_get_exec (entry), &args_length1, &args, &_inner_error_);
		if (_inner_error_ != NULL) {
				...
				while (TRUE) {
					...
					if (exec) {
						...
						switch (0) {
							default:
							{
								result = g_strdup (gmenu_tree_entry_get_name (entry));
								_g_free0 (basename);
								_g_string_free0 (sb);
								args = (_vala_array_free (args, args_length1, (GDestroyNotify) g_free), NULL);
								return result;
	...
	__catch1_g_error:
	{
		...
		_inner_error_ = NULL;
		{
			result = g_strdup (gmenu_tree_entry_get_exec (entry));
			_g_error_free0 (e);
			_g_string_free0 (sb);
			args = (_vala_array_free (args, args_length1, (GDestroyNotify) g_free), NULL);
			return result;
