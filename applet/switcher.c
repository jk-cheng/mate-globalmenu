...
static WnckWindow* switcher_item_to_window (Switcher* self, GnomenuMenuItem* item);
...
static void switcher_on_activate (Switcher* self, GnomenuShell* _this, GnomenuItem* item);
...
static GnomenuMenuItem* switcher_window_to_item (Switcher* self, WnckWindow* window);
...
static void _switcher_on_activate_gnomenu_shell_activate (Switcher* _sender, GnomenuItem* item, gpointer self);
...
GnomenuMenuItem* search_box_menu_item_find_item (SearchBoxMenuItem* self);
...
static void switcher_on_activate (Switcher* self, GnomenuShell* _this, GnomenuItem* item) {
	...
	window = switcher_item_to_window (self, (_tmp0_ = item, GNOMENU_IS_MENU_ITEM (_tmp0_) ? ((GnomenuMenuItem*) _tmp0_) : NULL));
	if (window == NULL) {
		GnomenuShell* parent_shell;
		GnomenuItem* parent_item;
		WnckWindow* _tmp2_;
		GnomenuItem* _tmp1_;
		parent_shell = _g_object_ref0 (gnomenu_item_get_shell (item));
		parent_item = _g_object_ref0 (gnomenu_shell_get_owner (parent_shell));
		window = (_tmp2_ = switcher_item_to_window (self, (_tmp1_ = parent_item, GNOMENU_IS_MENU_ITEM (_tmp1_) ? ((GnomenuMenuItem*) _tmp1_) : NULL)), _g_object_unref0 (window), _tmp2_);
		_g_object_unref0 (parent_item);
		_g_object_unref0 (parent_shell);
	}
	_tmp3_ = gnomenu_item_get_item_id (item);
...
static void switcher_update (Switcher* self, gboolean include_menu) {
	...
	if (include_menu) {
		if (self->priv->_show_window_list) {
			...
			_g_free0 (_tmp13_);
			{
				gnomenu_parser_parse ((GnomenuShell*) self, s, &_inner_error_);
			...
			menu = _g_object_ref0 (gtk_menu_item_get_submenu ((GtkMenuItem*) self->priv->_label_item));
			{
				...
				for (widget_it = widget_collection; widget_it != NULL; widget_it = widget_it->next) {
					...
					widget = _g_object_ref0 ((GtkWidget*) widget_it->data);
					{
						...
						WnckWindow* window;
						item = _g_object_ref0 ((_tmp15_ = widget, GNOMENU_IS_MENU_ITEM (_tmp15_) ? ((GnomenuMenuItem*) _tmp15_) : NULL));
						if (item == NULL) {
							_g_object_unref0 (item);
							_g_object_unref0 (widget);
							continue;
						}
						gnomenu_menu_item_set_max_width_chars (item, 30);
...
static GnomenuMenuItem* switcher_window_to_item (Switcher* self, WnckWindow* window) {
	...
	char* _tmp1_;
	char* _tmp0_;
	GnomenuMenuItem* _tmp2_;
	g_return_val_if_fail (self != NULL, NULL);
	g_return_val_if_fail (window != NULL, NULL);
	result = (_tmp2_ = gnomenu_menu_bar_get ((GnomenuMenuBar*) self, _tmp1_ = g_strconcat ("/switcher/XID", _tmp0_ = g_strdup_printf ("%lu", wnck_window_get_xid (window)), NULL)), _g_free0 (_tmp1_), _g_free0 (_tmp0_), _tmp2_);
	return result;
}
...
static WnckWindow* switcher_item_to_window (Switcher* self, GnomenuMenuItem* item) {
	...
	id = g_strdup (gnomenu_item_get_item_id ((GnomenuItem*) item));
...
static void switcher_real_size_request (GtkWidget* base, GtkRequisition* req) {
	...
	gboolean _tmp0_ = FALSE;
	self = (Switcher*) base;
	GTK_WIDGET_CLASS (switcher_parent_class)->size_request ((GtkWidget*) GNOMENU_MENU_BAR (self), req);
...
static GObject * switcher_constructor (GType type, guint n_construct_properties, GObjectConstructParam * construct_properties) {
	...
	_inner_error_ = NULL;
	{
		...
		char* _tmp7_;
		{
			...
			self->priv->_label_item = (_tmp2_ = (_tmp1_ = gnomenu_shell_get_item ((GnomenuShell*) self, 0), GNOMENU_IS_MENU_ITEM (_tmp1_) ? ((GnomenuMenuItem*) _tmp1_) : NULL), _g_object_unref0 (self->priv->_label_item), _tmp2_);
			gtk_menu_shell_append ((GtkMenuShell*) self->priv->_label_item->_submenu_cache, (GtkWidget*) ((GtkMenuItem*) self->priv->search_box));
			g_signal_connect_object ((GnomenuShell*) self, "activate", (GCallback) _switcher_on_activate_gnomenu_shell_activate, self, 0);
			_g_free0 (s);
		}
		goto __finally28;
		__catch28_g_error:
...
GType switcher_get_type (void) {
	...
	if (g_once_init_enter (&switcher_type_id__volatile)) {
		...
		switcher_type_id = g_type_register_static (GNOMENU_TYPE_MENU_BAR, "Switcher", &g_define_type_info, 0);
...
GnomenuMenuItem* search_box_menu_item_find_item (SearchBoxMenuItem* self) {
	...
	GnomenuShell* shell;
	g_return_val_if_fail (self != NULL, NULL);
	shell = _g_object_ref0 ((_tmp0_ = gtk_widget_get_parent ((GtkWidget*) self), GNOMENU_IS_SHELL (_tmp0_) ? ((GnomenuShell*) _tmp0_) : NULL));
	{
		gint i;
		i = 0;
		{
			...
			while (TRUE) {
				...
				item = (_tmp2_ = gnomenu_shell_get_item (shell, i), GNOMENU_IS_MENU_ITEM (_tmp2_) ? ((GnomenuMenuItem*) _tmp2_) : NULL);
				if (gnomenu_item_get_item_label ((GnomenuItem*) item) != NULL) {
					char* _tmp5_;
					char* _tmp4_;
					_tmp3_ = g_str_has_prefix (_tmp4_ = g_utf8_casefold (gnomenu_item_get_item_label ((GnomenuItem*) item), -1), _tmp5_ = g_utf8_casefold (gtk_entry_get_text (self->priv->textbox), -1));
					_g_free0 (_tmp5_);
					_g_free0 (_tmp4_);
				} else {
					_tmp3_ = FALSE;
				...
				if (_tmp3_) {
					GnomenuMenuItem* _tmp6_;
					result = _g_object_ref0 ((_tmp6_ = item, GNOMENU_IS_MENU_ITEM (_tmp6_) ? ((GnomenuMenuItem*) _tmp6_) : NULL));
					_g_object_unref0 (item);
...
static void _lambda1_ (GtkEntry* box, SearchBoxMenuItem* self) {
	...
	GnomenuMenuItem* item;
	g_return_if_fail (box != NULL);
	shell = _g_object_ref0 ((_tmp0_ = gtk_widget_get_parent ((GtkWidget*) self), GNOMENU_IS_MENU (_tmp0_) ? ((GnomenuMenu*) _tmp0_) : NULL));
