...
extern GnomenuSettings* settings;
GnomenuSettings* settings = NULL;
extern gulong xid;
gulong xid = (gulong) 0;
...
gint _vala_main (char** args, int args_length1) {
	...
	klass = (_tmp0_ = (GObjectClass*) g_type_class_ref (GNOMENU_TYPE_SETTINGS), _g_type_class_unref0 (klass), _tmp0_);
	...
	if (xid == 0) {
		GnomenuSettings* _tmp1_;
		settings = (_tmp1_ = (GnomenuSettings*) gnomenu_global_settings_get (gdk_screen_get_default ()), _g_object_unref0 (settings), _tmp1_);
	} else {
		GnomenuSettings* _tmp2_;
		settings = (_tmp2_ = (GnomenuSettings*) gnomenu_local_settings_new (gdk_window_foreign_new ((GdkNativeWindow) xid)), _g_object_unref0 (settings), _tmp2_);
	}
	if (show) {
		list_settings ();
		result = 0;
	...
	gnomenu_settings_push (settings);
	gdk_flush ();
	result = 0;
	_g_option_context_free0 (context);
	return result;
